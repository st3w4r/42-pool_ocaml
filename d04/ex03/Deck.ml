...
let toStringList deck = (
		let rec print_lst lst acc = match lst with
			| hd::tl -> print_lst tl (List.append acc [Card.toString hd])
			| [] -> acc
		in print_lst deck []
	)
...
let toStringListVerbose deck = (
		let rec print_lst lst acc = match lst with
			| hd::tl -> print_lst tl (List.append acc [Card.toStringVerbose hd])
			| [] -> acc
		in print_lst deck []
	)

let drawCard deck = (
		let rec draw_first_card lst = match lst with
			| hd::tl -> (hd, tl)
			| [] -> invalid_arg ("Deck empty")
		in draw_first_card deck
	)
